...
for file in os.listdir('index'):
        if file.endswith('.txt'):
            file_path = './index/' + file  # Prepend the directory name here
            loader = TextLoader(file_path, encoding='UTF-8')
            all_text.extend(loader.load())   

txt_files = [file for file in os.listdir('./data') if file.endswith('.txt')]
if txt_files:
    random_file_name = random.choice(txt_files)
    random_file_path = './data/' + random_file_name
    loader = TextLoader(random_file_path, encoding='UTF-8')
    text = loader.load()
...
all_text = text_splitter.split_documents(all_text)
print(all_text)
# print(text)
...
for i in range(5):
    ...
    re = qa.run({f"query": user_input})
    # response = qa({"query": user_input}, return_only_outputs=True)
    # answer = response['result']
    print(re)  # Changed 're' to 'answer' to print the response
    # return re
